 ...
 * 
 */
// 使用注解的方式 ( http://127.0.0.1:8080/servlet3/HttpGetServlet )
@WebServlet(name="HttpGetServlet", urlPatterns={"/HttpGetServlet"})
public final class HttpGetServlet extends BaseServlet
{
	 /* long */
...
/*		String path = WebPathUtil.getWebPath(getClass());
			...
			throws ServletException, IOException
	{
		...
		//log.info("doGet =====> Content-Type = " + request.getHeader("Content-Type"));
		
		//EnumUtil.print(request.getHeaderNames());
		
		//EnumUtil.print(request.getHeaders("Accept"));
		...
		while (accepts.hasMoreElements())
		{
			...
			{
				result = objectWriter.writeValueAsString(user);
				
				break;
			} else if ("application/xml".equals(accept))
			{
				result = xstream.toXML(user);
